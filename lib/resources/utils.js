...
function upload(token, pathSegments, filepath, options, cb) { //cb(error)
    ...
    var url = 'https://' + options.pivotalHost + '/services/v5/' + path + '?token=' + token;

    function up(url, file, opts) {
        var r = request.post(url, {}, function(error, res, body) {
    ...
    if (Buffer.isBuffer(filepath)) {
        var magic = new Magic(mmm.MAGIC_MIME_TYPE);
            magic.detect(filepath, function(err, type) {
                up(url, filepath, {
                    filename: 'image.' + type.replace('image/', ''),
                    contentType: type,
                    knownLength: filepath.length
                });
            });
    } else {
        up(url, filepath);
